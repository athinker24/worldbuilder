...
Closed source, free to use. The full terms are in [TERMS.txt](TERMS.txt), which the installer
shows as a page you must accept and which ships beside the executable in the portable zip too.
...
- [PRIVACY.txt](PRIVACY.txt) — the app makes no network requests, has no accounts and collects
  nothing. It also says what the log records, and what is stripped from a `.world` you share.
- [THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt) — the open source components the app is built
  from, each under its own licence, reproduced in full. Generated by `node gen-notices.mjs`; run
  it after changing dependencies, or the release gate will fail.

All three are opened from **Help ▸** inside the app, in both distributions.
