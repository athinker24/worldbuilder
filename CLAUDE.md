...
- `node gen-notices.mjs` — regenerates `THIRD-PARTY-NOTICES.txt` from the installed dependency tree. Run it after adding, removing or upgrading a dependency. The release gate runs it and fails if the file changes, because a stale notice file breaks nothing and ships anyway.
