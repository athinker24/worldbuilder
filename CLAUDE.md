...
- `node tests/db.test.ts` — the MAIN side's test harness: schema setup + CRUD + undo + the security scenarios, 300 assertions. There is no test framework; this file stands in for one and must be run after any data-model change. It was the last 2155 lines of `db.ts` until it moved out here, and the reason is worth keeping: its guard was `process.argv[1]`, i.e. a RUNTIME test, so no bundler could eliminate it and every packaged build shipped the whole harness inside `out/main/index.js`. It reaches nine module-private things in `db.ts` through one export, `__test`, which nothing under `src/` may import — asserted, because `__test.db` is the raw connection and an application-side import would walk straight past the write gates.
...
input, so `M` is guarded by the typing check plus "no modifier held". `File > Close Project` and `New Project` run the SAME function (`newProject`): there is no third state where a project is closed but the working copy still holds it. **`Overview.tsx`** is a fragment, not a wrapper `.page` — each tab child renders its own `.page`, and nesting them would double the padding and hand the scroll to the wrong element. Main carries its own small `MAIN_TR` map, because it cannot import the renderer's `i18n.tsx` — and it is NOT menu-only despite the `ml` name: the menu is the bulk of it, but the native open-failure dialogs, the log-folder warning and the default `New map` name come through the same table. Anything MAIN shows belongs there; anything the renderer shows belongs in `i18n.tsx`. **That includes the native dialogs** — save/open/close, the startup failures, the legal-file warning and their BUTTONS all go through `ml`, which takes `{name}` params like `t` does.
